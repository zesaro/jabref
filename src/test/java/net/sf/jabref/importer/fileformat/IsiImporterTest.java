...
public class IsiImporterTest {
    ...
    public void testIsRecognizedFormatRejected() throws IOException {
        ...
        IsiImporter importer = new IsiImporter();
        List<String> list = new LinkedList<>();
        list.add("IsiImporterTestWrong1.isi");
        list.add("IsiImporterTestWrong2.isi");
        list.add("Empty.txt");
        list.add("IsiImporterTestEmpty.isi");
    ...
    public void testIEEEImport() throws IOException {
        ...
        try (InputStream is = IsiImporterTest.class.getResourceAsStream("IEEEImport2.txt")) {
            ...
            Assert.assertEquals(a.getType().getName(), BibtexEntryTypes.ARTICLE, a.getType());
            Assert.assertEquals("Geoscience and Remote Sensing Letters, IEEE", a.getField("journal"));
            Assert.assertEquals(null, a.getField("title"));
    ...
    @Test
    public void testImportEntriesWrong() throws IOException {
        IsiImporter importer = new IsiImporter();

        List<BibtexEntry> entry1 = importer.importEntries(
                IsiImporterTest.class.getResourceAsStream("IsiImporterTestWrong1.isi"), new OutputPrinterToNull());
        List<BibtexEntry> entry2 = importer.importEntries(
                IsiImporterTest.class.getResourceAsStream("IsiImporterTestWrong2.isi"), new OutputPrinterToNull());

        Assert.assertEquals(1, entry1.size());
        Assert.assertEquals(1, entry2.size());
    }
    ...
    @Test
    public void testImportEntriesEmpty2() throws IOException {
        IsiImporter importer = new IsiImporter();

        try (InputStream is = IsiImporterTest.class.getResourceAsStream("Empty.txt")) {
